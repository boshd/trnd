...
public class SwiftyGiphyViewController: UIViewController {
    ...
    public override func viewWillLayoutSubviews() {
        ...
        if #available(iOS 11, *)
        {
            collectionView.contentInset = UIEdgeInsets(top: 44.0, left: 0.0, bottom: 10.0, right: 0.0)
            collectionView.scrollIndicatorInsets = UIEdgeInsets(top: 44.0, left: 0.0, bottom: 10.0, right: 0.0)
        }
        else
        {
            collectionView.contentInset = UIEdgeInsets(top: topLayoutGuide.length + 44.0, left: 0.0, bottom: 10.0, right: 0.0)
            collectionView.scrollIndicatorInsets = UIEdgeInsets(top: topLayoutGuide.length + 44.0, left: 0.0, bottom: 10.0, right: 0.0)
        }
    }
...
extension SwiftyGiphyViewController: UICollectionViewDelegate {
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        //print("out \(String(describing: selectedGif.url))")
        // saftely unwrap plz

        print(selectedGif.url!)
            //print("in \(selectedGif.url)")
        ...
        DispatchQueue.global(qos: .default).async(execute: {
            ...
            DispatchQueue.main.async(execute: {
                //UI Update will here
                NavigationManager.showPreviewViewController(withPresenter: self, withGifUrl: selectedGif.url!)
            })
        })
        searchController.isActive = false
